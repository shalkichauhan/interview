...
done....Two Sum: Given an array of integers, return indices of the two numbers such that they add up to a specific target.
done.... Longest Substring Without Repeating Characters: Find the length of the longest substring without repeating characters.
done...Maximum Subarray: Find the contiguous subarray which has the largest sum and return its sum.
done....Container With Most Water: Find two lines which together with the x-axis form a container that holds the most water.
...
done...Reverse Linked List: Reverse a singly linked list.
done...Merge Two Sorted Lists: Merge two sorted linked lists and return it as a new list.
...
function DSA_questionsTs(str:string){
    let maxSubStr:number = 0; let finalObj
    ...
    let arrayString = emptyArr.toString()
    const Finaloutput = arrayString.split(',').join('')
    return [maxSubStr,Finaloutput]
}console.log(DSA_questionsTs("abcdefabcf"))
